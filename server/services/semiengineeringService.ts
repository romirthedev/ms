...
async function fetchRss(): Promise<{ title: string; link: string; pubDate?: Date }[]> {
  const url = 'https://semiengineering.com/feed/'
...
async function updateSemiengineering(): Promise<{ ingested: number }> {
...
export const semiengineeringService = { updateSemiengineering }