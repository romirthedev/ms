...
async function fetchLobstersRss(): Promise<{ title: string; link: string; pubDate?: Date }[]> {
...
async function updateLobsters(): Promise<{ ingested: number }> {
  if (!symbolCache) await buildSymbolCache()
  let ingested = 0
  const feed = await fetchLobstersRss()
...
export const lobstersService = { updateLobsters }