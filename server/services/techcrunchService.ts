...
async function fetchTechcrunchRss(): Promise<{ title: string; link: string; pubDate?: Date }[]> {
...
async function updateTechcrunch(): Promise<{ ingested: number }> {
  if (!symbolCache) await buildSymbolCache()
  let ingested = 0
  const feed = await fetchTechcrunchRss()
...
export const techcrunchService = { updateTechcrunch }