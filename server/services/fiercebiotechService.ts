...
async function fetchRss(): Promise<{ title: string; link: string; pubDate?: Date }[]> {
  const url = 'https://www.fiercebiotech.com/rss.xml'
...
async function updateFiercebiotech(): Promise<{ ingested: number }> {
...
export const fiercebiotechService = { updateFiercebiotech }