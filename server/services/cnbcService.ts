...
async function fetchCnbcRss(): Promise<{ title: string; link: string; pubDate?: Date }[]> {
...
async function updateCnbc(): Promise<{ ingested: number }> {
  if (!symbolCache) await buildSymbolCache()
  let ingested = 0
  const feed = await fetchCnbcRss()
...
export const cnbcService = { updateCnbc }