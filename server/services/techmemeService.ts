...
async function fetchTechmemeFeed(): Promise<{ title: string; link: string; pubDate?: Date }[]> {
  const url = 'https://techmeme.com/feed.xml'
...
async function upsert(item: { title: string; link: string; pubDate?: Date }): Promise<boolean> {
  ...
  const dup = existing.find(n => n.url === item.link)
  if (dup) return false
...
async function updateTechmeme(): Promise<{ ingested: number }> {
  if (!symbolCache) await buildSymbolCache()
  let ingested = 0
  const feed = await fetchTechmemeFeed()
...
export const techmemeService = { updateTechmeme }