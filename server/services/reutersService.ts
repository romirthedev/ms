...
async function fetchReutersRss(): Promise<{ title: string; link: string; pubDate?: Date }[]> {
  const url = 'https://www.reuters.com/markets/rss'
...
async function updateReuters(): Promise<{ ingested: number }> {
  if (!symbolCache) await buildSymbolCache()
  let ingested = 0
  const feed = await fetchReutersRss()
...
export const reutersService = { updateReuters }