...
function topicToUrl(topic: string): string {
  const q = encodeURIComponent(topic)
  return `https://news.google.com/rss/search?q=${q}&hl=en-US&gl=US&ceid=US:en`
}

async function fetchTopic(topic: string): Promise<{ title: string; link: string; pubDate?: Date }[]> {
...
async function updateGoogleNews(): Promise<{ ingested: number }> {
  ...
  const topics = [
    ...
    'electronics manufacturing sensor display battery'
  ]
  for (const t of topics) {
    const feed = await fetchTopic(t)
